...
def parse_arguments():
    ...
    parser.add_argument(
        "--skipped_cases",
        help="Shows only the test cases in which the result was not evaluated",
        action="store_true",
    )
    parser.add_argument(
        "--success_cases",
    ...
    only_skipped = args.skipped_cases
...
def create_asset_for_runtime(api_client, asset, id, helper_name, failed_tests):
    ...
    error_message = f"Expected an error response, but received {'Success' if response.status == api_engine.OK else 'Error'}"
    failed_tests.append(
        {
            "helper": helper_name,
            "id": id,
            "description": f"Expected True, but got {response}",
        }
    )
    return False
...
def generate_report(successful_tests, failed_tests, skipped_tests):
    global only_success
    global only_failure
    global only_skipped
    ...
    report += f"- Total test cases executed: {len(successful_tests) + len(failed_tests) + len(skipped_tests)}\n"
    report += f"- Successful test cases: {len(successful_tests)}\n"
    report += f"- Failed test cases: {len(failed_tests)}\n"
    report += f"- Skipped test cases: {len(skipped_tests)}\n\n"
    ...
    if only_skipped:
        if skipped_tests:
            ...
            for i, skipped_test in enumerate(skipped_tests, start=1):
                report += f"{i}. **Test Case {i}**\n"
                report += f"   - Helper: {skipped_test['helper']}\n"
                report += f"   - Id: {skipped_test['id']}\n"
...
def handle_response(
    id, expected, response, helper_name, successful_tests, failure_tests
):
    if (expected and response.status == api_engine.OK) or (
        ...
        if expected:
            error_message = f"Expected a successful response, but received {'Success' if response.status == api_engine.OK else 'Error'}"
        else:
            error_message = f"Expected an error response, but received {'Success' if response.status == api_engine.OK else 'Error'}"
        failure_tests.append(
            {"helper": helper_name, "id": id, "description": error_message}
        )
...
def extract_transformation_result_from_response(response):
    response = json.loads(MessageToJson(response))
    regex = r"target_field: (?P<function>\w+)\((?P<arguments>[^)]+)\) -> (?P<result>Success|Failure)"
...
def handle_test_result(
    ...
    if expected != None:
                ...
                failure_test.append(
                    {
                        "helper": helper_name,
                        "id": id,
                        "description": f"Expected {'True' if expected else 'False'}, but got {result}",
                    }
                )
                ...
                failure_test.append(
                    {
                        "helper": helper_name,
                        "id": id,
                        "description": f"{'Success' if expected else 'Error'}, but got {result}",
                    }
                )
        else:
        ...
        skipped_test.append({"helper": helper_name, "id": id})


def create_asset_for_buildtime(
    api_client, id, asset, helper_name, expected, successful_tests, failure_test
):
    request = build_asset_request(asset)
    error, response = send_recv(
        api_client, request, api_engine.GenericStatus_Response()
    )
    handle_response(id, expected, response, helper_name, successful_tests, failure_test)
...
def run_test_cases_executor(api_client, socket_path):
    ...
    generate_report(successful_tests, failure_tests, skipped_tests)
